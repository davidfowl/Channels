...
namespace Channels.Samples.IO
{
    public class ReadableFileChannel : ReadableChannel
    {
        ...
        public unsafe static void IOCallback(uint errorCode, uint numBytes, NativeOverlapped* pOverlapped)
        {
            ...
            if (numBytes == 0 || operation.Channel.Input.Completion.IsCompleted)
            {
                operation.Channel.Output.CompleteWriting();
        ...
        private class ReadOperation
        {
            ...
            {
                var buffer = Channel.Output.Alloc(2048);
                ...
                if (hr != 997)
                {
                    Channel.Output.CompleteWriting(Marshal.GetExceptionForHR(hr));
                }
            }
