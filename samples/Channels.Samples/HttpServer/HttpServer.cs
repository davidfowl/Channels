...
namespace Channels.Samples.Http
{
    public class HttpServer : IServer
    {
        ...
        private void StartAcceptingLibuvConnections<TContext>(IHttpApplication<TContext> application, IPAddress ip, int port)
        {
            ...
            {
                await ProcessClient(_uvThread.ChannelFactory, application, connection);
            });
        ...
        private static async Task ProcessRIOConnection<TContext>(IHttpApplication<TContext> application, RioTcpConnection connection)
        {
            using (connection)
            {
                await ProcessClient(connection.ChannelFactory, application, connection);
            }
        }
        ...
        private static async Task ProcessConnection<TContext>(IHttpApplication<TContext> application, ChannelFactory channelFactory, Socket socket)
        {
            using (var ns = new NetworkStream(socket))
            {
                ...
                {
                    await ProcessClient(channelFactory, application, channel);
                }
            }
        }

        private static async Task ProcessClient<TContext>(ChannelFactory factory, IHttpApplication<TContext> application, IChannel channel)
        {
            var connection = new HttpConnection<TContext>(factory, application, channel.Input, channel.Output);
