...
namespace Channels.Networking.TLS.Internal.OpenSsl
{
    internal unsafe static class Interop
    {
        ...
        public static IntPtr NewServerContext(VerifyMode verifyMode) => IsWindows ? WindowsLib.NewServerContext(verifyMode) : UnixLib.NewServerContext(verifyMode);
        public static IntPtr NewClientContext(VerifyMode verifyMode) => IsWindows ? WindowsLib.NewClientContext(verifyMode) : UnixLib.NewClientContext(verifyMode);
        ...
        public static int SSL_write(IntPtr ssl, void* buf, int len) => IsWindows ? WindowsLib.SSL_write(ssl, buf, len) : UnixLib.SSL_write(ssl, buf, len);
        public static int SSL_read(IntPtr ssl, void* buf, int len) => IsWindows ? WindowsLib.SSL_read(ssl, buf, len) : UnixLib.SSL_read(ssl, buf, len);
        public static int SSL_do_handshake(IntPtr ssl) => IsWindows ? WindowsLib.SSL_do_handshake(ssl) : UnixLib.SSL_do_handshake(ssl);
        public static SslErrorCodes SSL_get_error(IntPtr ssl, int errorIndetity) => IsWindows ? WindowsLib.SSL_get_error(ssl, errorIndetity) : UnixLib.SSL_get_error(ssl, errorIndetity);
        public static int SetKeys(IntPtr ctx, IntPtr certificates, IntPtr privateKey) => IsWindows ? WindowsLib.SetKeys(ctx, certificates, privateKey) : UnixLib.SetKeys(ctx, certificates, privateKey);
        ...
        public static IntPtr SSL_new(IntPtr sslContext) => IsWindows ? WindowsLib.SSL_new(sslContext) : UnixLib.SSL_new(sslContext);
        ...
        public static int SSL_CTX_set_alpn_protos(IntPtr ctx, IntPtr protocolList, uint protocolListLength) => IsWindows ? WindowsLib.SSL_CTX_set_alpn_protos(ctx, protocolList, protocolListLength) : UnixLib.SSL_CTX_set_alpn_protos(ctx, protocolList, protocolListLength);
